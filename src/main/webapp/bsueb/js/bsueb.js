...
//    }
  ...
  function initLb(lbname) {
    if ($('#' + lbname).val() === null) {
      ...
      //"listboxen": [{"name": "lotyplb", "typ": "appKonst", "table": "V_AC_FLSLAGERORTTYP"},           
      var url = "../Listbox?typ=";
  ...
  speicherncust = function speicherncust(tabelle) {
    ...
    $('.custausgabe').each(function (index, value) {
      let _id = value.id;
      UTIL.logger(dialogname + ": speicherncust(): index: " + index
        + ";  chekbox id: " + _id + ': chekbox value: ' + value.checked);
      if (!value.checked) {
